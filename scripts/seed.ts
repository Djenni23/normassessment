...
async function main() {
  ...
  if (count === 0) {
    const demos: Array<{ ref: string; name: string; country: string; type: string; typeIcon: string; projectType: string; cat: string; equipment: EquipmentInput; status: string; daysAgo: number }> = [
      ...
      { ref: "NE-2026-9L7C", name: "Jean-Paul Kouassi", country: "Côte d’Ivoire", type: "School", typeIcon: "school", projectType: "school", cat: "Commercial", equipment: { lights: { qty: 20 }, computer: { qty: 8 }, fan: { qty: 6 }, internet: { qty: 1 }, camera: { qty: 2 } }, status: "Quoted", daysAgo: 14 },
